...
#ifndef PARALLELPARTITIONDYN_BASE_CPP
#define PARALLELPARTITIONDYN_BASE_CPP
...
namespace myParallelPartition {
	...
	struct Parallel_Partitioner {
		std::mutex work_mtx;
		iter l_next, r_next;
		uint64_t elem_per_block;
		uint64_t size;
  	iter* right_rem, *left_rem;

  	Parallel_Partitioner(uint16_t noThreads, uint64_t noElemPerBlock, iter lNext, iter rNext) {
			size = noThreads;
    ...
    inline void parallel_partition(const iter& _l_begin, const iter& _l_end,
				...
				Comp pred, const uint16_t thNo, const uint16_t blockSize) {

			//std::cout << "sizes: left: " << (_l_end-_l_begin) << " right: " << (_r_begin-_r_end) << std::endl;

			typedef typename std::iterator_traits<iter>::value_type T;
			T pivot = *pivot_iter; // Pivot might get swapped
			T temp;
			//const uint32_t size = PP_BLOCKSIZE+(l_end-l_begin);
			iter lOut[blockSize]; // Direct writeout to struct
			iter rOut[blockSize];
			int64_t lc = 0;
			int64_t rc = 0;
			...
			while((li!=l_end||rc>0) && (ri!=r_end||lc>0)) {
				...
				while((lc<blockSize-15)&&(ri>(r_end+15))) {
				...
				while((lc<blockSize)&&(ri!=r_end)) {
					checkAndAdvanceToLeft(ri, lResult, pivot, lOut, lc)
				}

				while((rc<blockSize-15)&&(li<(l_end-15))) {
		...
		inline void neutralizeRemainder(const iter& pivot_iter, Comp pred, const uint16_t blockSize) {
			int thNo = 0;
			typedef typename std::iterator_traits<iter>::value_type T;
			T pivot = *pivot_iter;
			T temp;
