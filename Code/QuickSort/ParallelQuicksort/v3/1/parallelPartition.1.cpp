...
#include "./parallelPartitionBase.1.cpp"


#ifndef PARALLELPARTITION_1_CPP
#define PARALLELPARTITION_1_CPP
...
namespace myPartitionA {
    ...
    // Find middle block
    volatile int32_t midOffL = (partitioner.l_next-begin); // Swap from (here - block size) including
    volatile int32_t midOffR = (partitioner.r_next-begin)+1; // Swap from here including (+1: Leave last elem of mid)

    // Swap remaining blocks to middle block
    typedef typename std::iterator_traits<iter>::value_type T;
    uint32_t test = sizeof(T);
		void* swpBuff = malloc(elemPerSide*sizeof(T));
		uint32_t leftRemCnt = 0;
		uint32_t rightRemCnt = 0;
