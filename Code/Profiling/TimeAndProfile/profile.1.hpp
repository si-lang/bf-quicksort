...
struct PerfEvents {
   ...
   std::unordered_map<std::string, std::vector<event>> events;
	 std::unordered_map<std::string, double> counterBuffer;
   ...
   PerfEvents() {
      ...
      if (cpu == "GenuineIntel-6-57-core") {
         ...
         add("cycles", "cpu/cpu-cycles/");
         add("LLC-misses", "cpu/cache-misses/");
         add("l1-misses", PERF_TYPE_HW_CACHE, PERF_COUNT_HW_CACHE_L1D | (PERF_COUNT_HW_CACHE_OP_READ << 8) | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16));
         add("instructions", "instructions");
      ...
      } else if (cpu == "GenuineIntel-6-25-core") {
         // Simon's Schnaufal
         add("cycles", "cpu/cpu-cycles/");
         add("LLC-misses", "cpu/cache-misses/");
         add("l1-misses", PERF_TYPE_HW_CACHE, PERF_COUNT_HW_CACHE_L1D | (PERF_COUNT_HW_CACHE_OP_READ << 8) | (PERF_COUNT_HW_CACHE_RESULT_MISS << 16));
   ...
   void readAll() {
      for (auto& ev : events)
         ...
         for (auto& event : events[name])
            counterBuffer[name] += event.readCounter();
      }
   }
   ...
   void printAll(std::ostream& out, double n) {
      for (auto& name : ordered_names) {
         printf("%8.2f,", counterBuffer[name] / n);
				 counterBuffer[name] = 0;
      }
   }
   ...
   void timeAndProfile(std::string s, uint64_t count, std::function<void()> fn, uint64_t repetitions = 1, std::vector<std::pair<std::string,std::string>>&& extraStrings = {});
	 void timeAndProfile(std::string s, uint64_t count, std::function<void()> fn, std::function<void(size_t i)> reset, uint64_t repetitions = 1, std::vector<std::pair<std::string,std::string>>&& extraStrings = {});
};
...
void PerfEvents::timeAndProfile(std::string s, uint64_t count, std::function<void()> fn, uint64_t repetitions, std::vector<std::pair<std::string,std::string>>&& extraStrings) {
   ...
   if (writeHeader) {
      ...
      printf("  timems, CPUtime,     IPC,     GHz,    BWrd,  cycles, LLCmiss,  L1miss, instruc,  brmiss,  all_rd,  stores,   loads, taskclk,\n");
   }
   ...
   printf("%8.2f,", ((*this)["cycles"] / (this->events["cycles"][0].data.time_enabled - this->events["cycles"][0].prev.time_enabled)));
...
void PerfEvents::timeAndProfile(std::string s, uint64_t count, std::function<void()> fn, std::function<void(size_t i)> reset, uint64_t repetitions, std::vector<std::pair<std::string,std::string>>&& extraStrings) {
   // warmup round
   /*if (repetitions>4) {
      fn();
      repetitions--;
   ...
   if (writeHeader) {
      ...
      for (auto& p : extraStrings)
         printf("%8s,", p.first.c_str());
      printf("  timems, timemsA, CPUtime,     IPC,     GHz,    BWrd,  cycles, LLCmiss,  L1miss, instruc,  brmiss,  all_rd,  stores,   loads, taskclk,\n");
   }
   ...
   for (auto& p : extraStrings)
      printf("%8s,", p.second.c_str());
   printf("%8.2f,", (runtime * 1e3) / repetitions);
	 printf("%8.2f,", (runtime * 1e3));
   printf("%8.2f,", ((*this)["task-clock"] / (runtime * 1e9)));
   printf("%8.2f,", ((*this)["instructions"] / (*this)["cycles"]));
   printf("%8.2f,", ((*this)["cycles"] / (this->events["cycles"][0].data.time_enabled - this->events["cycles"][0].prev.time_enabled)));
   printf("%8.0f,", ((((*this)["all_rd"] * 64.0) / (1024 * 1024)) / runtime));
