...
#ifndef _GLIBCXX_PARALLEL_PARTITION_MOD_H
#define _GLIBCXX_PARALLEL_PARTITION_MOD_H 1
...
namespace __gnu_parallel_mod
{
  ...
  template<typename _RAIter, typename _Predicate>
    ...
    __parallel_partition(_RAIter __begin, _RAIter __end,
			 _Predicate __pred, __gnu_parallel::_ThreadIndex __num_threads)
    {
      ...
      const __gnu_parallel::_Settings& __s = __gnu_parallel::_Settings::get();
...
#             pragma omp single
		...
		for (__gnu_parallel::_ThreadIndex __r = 0; __r < __num_threads; ++__r)
		  {
		  ...
		  if (__thread_left > __thread_left_border)
		    {
                      _DifferenceType __former_dist =
                              __gnu_parallel::__fetch_and_add(&__dist, -__chunk_size);
                      if (__former_dist < __chunk_size)
                        ...
                        {
                          __thread_left =
                                  __gnu_parallel::__fetch_and_add(&__left, __chunk_size);
		  ...
		  if (__thread_right < __thread_right_border)
		    {
                      ...
                      if (__former_dist < __chunk_size)
                        ...
                        {
                          __thread_right =
                                  __gnu_parallel::__fetch_and_add(&__right, -__chunk_size);
		  ...
		  _DifferenceType __swapstart = -1;
                  for (int __r = 0; __r < __leftover_left; ++__r)
                    if (__reserved_left[__r] == 0
                        && __gnu_parallel::__compare_and_swap(&(__reserved_left[__r]), 0, 1))
                      {
		  ...
		  _DifferenceType __swapstart = -1;
                  for (int __r = 0; __r < __leftover_right; ++__r)
                    if (__reserved_right[__r] == 0
                        && __gnu_parallel::__compare_and_swap(&(__reserved_right[__r]), 0, 1))
                      {
...
#           pragma omp flush(__left, __right)
  ...
  template<typename _RAIter, typename _Compare>
			   ...
			   _RAIter __end, _Compare __comp)
    {
      ...
      _RAIter __split;
      __gnu_parallel::_RandomNumber __rng;
      ...
      while (static_cast<__gnu_parallel::_SequenceIndex>(__end - __begin) >= __minimum_length)
	{
          _DifferenceType __n = __end - __begin;
      ...
      // Only at most __gnu_parallel::_Settings::partition_minimal_n __elements __left.
