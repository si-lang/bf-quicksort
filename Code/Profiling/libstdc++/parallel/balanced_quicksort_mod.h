...
#ifndef _GLIBCXX_PARALLEL_BALANCED_QUICKSORT_MOD_H
#define _GLIBCXX_PARALLEL_BALANCED_QUICKSORT_MOD_H 1
...
namespace __gnu_parallel_mod
{
  ...
  template<typename _RAIter>
    struct _QSBThreadLocal
    {
      ...
      __gnu_parallel::_RestrictedBoundedConcurrentQueue<_Piece> _M_leftover_parts;
  ...
  template<typename _RAIter, typename _Compare>
    ...
    __qsb_divide(_RAIter __begin, _RAIter __end,
		 _Compare __comp, __gnu_parallel::_ThreadIndex __num_threads)
    {
      ...
      _RAIter __pivot_pos =
	__gnu_parallel::__median_of_three_iterators(__begin, __begin + (__end - __begin) / 2,
				    __end  - 1, __comp);
	...
	__pred(__comp, *__pivot_pos);
      ...
      _DifferenceType __split_pos = __gnu_parallel_mod::__parallel_partition(__begin, __end - 1,
							 __pred,
							 __num_threads);
  ...
  template<typename _RAIter, typename _Compare>
    ...
    __qsb_conquer(_QSBThreadLocal<_RAIter>** __tls,
		  ...
		  _Compare __comp,
		  __gnu_parallel::_ThreadIndex __iam, __gnu_parallel::_ThreadIndex __num_threads,
		  bool __parent_wait)
    {
      typedef std::iterator_traits<_RAIter> _TraitsType;
      typedef typename _TraitsType::value_type _ValueType;
	...
	__num_threads_leftside = std::max<__gnu_parallel::_ThreadIndex>
	(1, std::min<__gnu_parallel::_ThreadIndex>(__num_threads - 1, __split_pos
				   * __num_threads / __n));
...
#         pragma omp section
  ...
  template<typename _RAIter, typename _Compare>
    ...
    __qsb_local_sort_with_helping(_QSBThreadLocal<_RAIter>** __tls,
				  _Compare& __comp, __gnu_parallel::_ThreadIndex __iam,
				  bool __wait)
    {
      typedef std::iterator_traits<_RAIter> _TraitsType;
      typedef typename _TraitsType::value_type _ValueType;
      ...
      _DifferenceType
	__base_case_n = __gnu_parallel::_Settings::get().sort_qsb_base_case_maximal_n;
      if (__base_case_n < 2)
	__base_case_n = 2;
      __gnu_parallel::_ThreadIndex __num_threads = __tl._M_num_threads;

      // Every thread has its own random number generator.
      __gnu_parallel::_RandomNumber __rng(__iam + 1);
...
#             pragma omp atomic
        	...
        	{
                  __gnu_parallel::_ThreadIndex __victim;
...
#                 pragma omp flush
  ...
  template<typename _RAIter, typename _Compare>
	...
	__num_threads = static_cast<__gnu_parallel::_ThreadIndex>(__n);

      // Initialize thread local storage
      _TLSType** __tls = new _TLSType*[__num_threads];
      _DifferenceType __queue_size = (__num_threads
				      * (__gnu_parallel::_ThreadIndex)(__gnu_parallel::__rd_log2(__n) + 1));
      for (__gnu_parallel::_ThreadIndex __t = 0; __t < __num_threads; ++__t)
	__tls[__t] = new _QSBThreadLocal<_RAIter>(__queue_size);

      // There can never be more than ceil(__gnu_parallel::__rd_log2(__n)) ranges on the
      // stack, because
      // 1. Only one processor pushes onto the stack
      // 2. The largest range has at most length __n
      // 3. Each range is larger than half of the range remaining
      volatile _DifferenceType __elements_leftover = __n;
      for (__gnu_parallel::_ThreadIndex __i = 0; __i < __num_threads; ++__i)
	{
      ...
      _Piece __dummy;
      for (__gnu_parallel::_ThreadIndex __i = 1; __i < __num_threads; ++__i)
